...
namespace Sogas 
{
	Material::Material(std::shared_ptr<Shader> shader)
		: m_pShader(shader), m_isTransparent(false), m_color(glm::vec4(1)), m_roughnessFactor(0.0f),
	...
	Material::Material(std::shared_ptr<Shader> shader, const MaterialProperties& properties)
		: m_pShader(shader), m_isTransparent(properties.isTransparent), m_color(properties.color),
